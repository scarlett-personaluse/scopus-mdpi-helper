...
// @version       1.1
// @icon64        https://www.google.com/url?sa=t&source=web&rct=j&url=https%3A%2F%2Fblog.mdpi.com%2F2022%2F09%2F29%2Fhistory-of-susy%2F&ved=0CBYQjRxqFwoTCLi5jKHL0ZQDFQAAAAAdAAAAABAF&opi=89978449
    ...
    GM_config.init({
        ...
        'fields': {
            ...
            'GE_ReminderID': {
                ...
                ['Guest Editor Invitation – Reminder (GE responsibility)', 'Guest Editor Invitation – Reminder (video call)', 'Reminder (general)', 'Reminder (with benefits)'], default: 'Guest Editor Invitation – Reminder (GE responsibility)'
            },
        ...
        function skSelectEmailTemplate(templateName) {
            let target = String(templateName || '').trim();
            let targetSimple = target.replace(/^Guest Editor Invitation\s*[–-]\s*/i, '').trim();
            let selected = false;

            $('#emailTemplates option').each(function () {
                let text = $(this).text().trim();

                if (
                    text === target ||
                    text.indexOf(target) > -1 ||
                    text.indexOf(targetSimple) > -1 ||
            ...
            if (!selected && target.indexOf('GE responsibility') > -1) {
                $("#emailTemplates > option:contains('GE responsibility'), #emailTemplates > option:contains('GE Responsibility'), #emailTemplates > option:contains('responsibility')").first().prop('selected', true);
            }

            if (!selected && target.indexOf('video call') > -1) {
                $("#emailTemplates > option:contains('video call'), #emailTemplates > option:contains('Video Call')").first().prop('selected', true);
            }

            if (!selected && target.indexOf('Guest Editor-Optional') > -1) {
                $("#emailTemplates > option:contains('Guest Editor-Optional'), #emailTemplates > option:contains('Optional')").first().prop('selected', true);
            }
        }
...
function onInit() {
        ...
        try {
            ...
            unsafeWindow.$(document.getElementById('emailTemplates')).append('<option value>*Guest Editor - SI Mentor Program</option>').trigger("chosen:updated").on("change", function () {
                if ($("#emailTemplates option:selected").text() == "*Guest Editor - SI Mentor Program") {
                    $.ajax({
                        url: $("#emailTemplates").attr("data-url"),
                        dataType: "json",
                        type: "post",
                        async: "true",
                        data: { id: "269", placeholders: $("#placeholders").val() },
                        success: function (data) {
                            let SI_mentor_body = data.body.replace(
                                /(Dear .*,)[\s\S]*tentative title of (.*) for the journal (.*), (.*)\)[\s\S]*(https:\/\/susy.mdpi.com\/guest_editor\/invitation\/process.*)[\s\S]*egards,/,
                            ...
                            $("#mailBody").val(SI_mentor_body);
                            $("#mailSubject").val(SI_mentor_subject);
                        }
                    });
                }
            });
            ...
            let geInviteParams = new URLSearchParams(window.location.search);
            let geInviteTemplate = GM_config.get('GE_TemplateID');

            // From SI page Invite (Quick): force the 5th GE invitation template: Guest Editor-Optional
            if (geInviteParams.has("Q") && geInviteParams.get("ge_template") === "optional") {
                geInviteTemplate = "Guest Editor-Optional";
            }

            skSelectEmailTemplate(geInviteTemplate);
            unsafeWindow.$(document.getElementById('emailTemplates')).trigger("chosen:updated").trigger("change");

            function init() {
                let t1 = skStringToRegex(GM_config.get('GE_TemplateS1'));
                $("#mailSubject").val($("#mailSubject").val().replace(t1, skStringToFunction(GM_config.get('GE_TemplateS2'))));

                let t2 = skStringToRegex(GM_config.get('GE_TemplateB1'));
                $("#mailBody").val($("#mailBody").val().replace(t2, skStringToFunction(GM_config.get('GE_TemplateB2'))));

                if (new URLSearchParams(window.location.search).has("Q")) {
                    setTimeout(function () {
                        $("#sendingEmail").trigger("click");
                    }, 500);
                }
            }
    ...
    if (window.location.href.indexOf("/remind/guest_editor") > -1) {
        try {
            ...
            let geReminderParams = new URLSearchParams(window.location.search);
            let geReminderTemplate = GM_config.get('GE_ReminderID');

            // From SI page Remind (Quick 1st): GE responsibility reminder
            if (geReminderParams.has("Q") && geReminderParams.get("ge_reminder_template") === "1") {
                geReminderTemplate = "Guest Editor Invitation – Reminder (GE responsibility)";
            }

            // From SI page Remind (Quick 2st): video call reminder
            if (geReminderParams.has("Q") && geReminderParams.get("ge_reminder_template") === "2") {
                geReminderTemplate = "Guest Editor Invitation – Reminder (video call)";
            }

            skSelectEmailTemplate(geReminderTemplate);
            unsafeWindow.$(document.getElementById('emailTemplates')).trigger("chosen:updated").trigger("change");

            function init() {
                let t1 = skStringToRegex(GM_config.get('GE_ReminderS1'));
                $("#mailSubject").val($("#mailSubject").val().replace(t1, skStringToFunction(GM_config.get('GE_ReminderS2'))));

                let t2 = skStringToRegex(GM_config.get('GE_ReminderB1'));
                $("#mailBody").val($("#mailBody").val().replace(t2, skStringToFunction(GM_config.get('GE_ReminderB2'))));
            ...
            $('#Awaiting').on("click", function () {
                if ($('#mailSubject').val().indexOf("Awaiting Your Reply") == -1) {
                    $('#mailSubject').val("Awaiting Your Reply: " + $('#mailSubject').val());
                }
            });
    ...
    if (window.location.href.indexOf(".mdpi.com/submission/topic/view") + window.location.href.indexOf(".mdpi.com/special_issue/process") > -2) {
        try {
            ...
            Quick_GE_Invite($("a[href^='/email/invite/guest_editor/']"));
            Quick_GE_Remind($("a[href^='/email/remind/guest_editor/']"));

            function skAddQueryParams(url, params) {
                let sep = url.indexOf("?") === -1 ? "?" : "&";
                return url + sep + params;
            }

            // Invite 后面的 Quick：自动选择第 5 个模板 Guest Editor-Optional
            function Quick_GE_Invite(param) {
                param.each(function () {
                    let quickUrl = skAddQueryParams($(this).attr("href"), "Q=1&ge_template=optional");
                    $(this).after(" (<a href='" + quickUrl + "'>Quick</a>)");
                });
            }

            // Remind 后面的 Quick：拓展为 Quick 1st / Quick 2st
            function Quick_GE_Remind(param) {
                param.each(function () {
                    let href = $(this).attr("href");

                    let quickUrl1 = skAddQueryParams(href, "Q=1&ge_reminder_template=1");
                    let quickUrl2 = skAddQueryParams(href, "Q=1&ge_reminder_template=2");

                    $(this).after(
                        " (<a href='" + quickUrl1 + "'>Quick 1st</a>" +
                        " · <a href='" + quickUrl2 + "'>Quick 2st</a>)"
                    );
                });
            }
    ...
    if (window.location.href.indexOf("//redmine.mdpi.") > -1 && GM_config.get('Cfp_checker')) {
        try {
            ...
            //CfP filter链接
            $("#header > h1").append(` <a href='https://redmine.mdpi.cn/projects/si-planning/issues?utf8=%E2%9C%93&set_filter=1&f[]=status_id&op[status_id]==&v[status_id][]=13&v[status_id][]=113&f[]=cf_10&op[cf_10]==&v[cf_10][]=` + GM_config.get('Journal')
