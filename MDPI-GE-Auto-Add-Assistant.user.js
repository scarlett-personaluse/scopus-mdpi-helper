...
// @version      3.2
// @description  Import GE CSV, load pending SI queue, add up to 5 proposed GE per SI per round, and skip no-proceed emails globally within the round.
// @author       Jiali Tang
...
(function () {
    ...
    const NO_PROCEED_TIMEOUT_TICKS = 40; // 40 * 250ms = 10s

    const LS_GE_POOL = "mdpi_ge_pool_v32";
    const LS_SI_QUEUE = "mdpi_si_queue_v32";
    const LS_RESULTS = "mdpi_ge_results_v32";
    const LS_RUNNING = "mdpi_ge_running_v32";
    const LS_AUTO_PROCEED = "mdpi_ge_auto_proceed_v32";
    const LS_LOG = "mdpi_ge_log_v32";
    const LS_SI_INDEX = "mdpi_si_index_v32";
    const LS_GE_INDEX = "mdpi_ge_index_v32";
    const LS_SI_COUNTS = "mdpi_si_round_counts_v32";
    const LS_CURRENT = "mdpi_current_task_v32";
    const LS_ROUND_SKIP_EMAILS = "mdpi_round_skip_emails_v32";
    ...
    function createPanel() {
        ...
        panel.innerHTML = `
            ...
            <div style="padding:12px;font-size:13px;">
                ...
                <div style="font-size:12px;color:#666;margin-bottom:8px;">
                    每一轮中，每个 SI 最多 Proceed 5 次。若某邮箱无 Proceed，本轮后续 SI 不再重复测试该邮箱。
                </div>
                ...
                <textarea id="gea-csv" placeholder="Paste CSV here if not using file upload." style="width:100%;height:95px;margin-top:8px;border:1px solid #ccc;border-radius:8px;padding:8px;font-size:12px;"></textarea>
        ...
        autoBox.onchange = () => {
            ...
            log(autoBox.checked ? "Auto Proceed ON." : "Auto Proceed OFF.");
            updateStatus();
        };
    ...
    function updateStatus(extra = "") {
        ...
        const skipEmails = getJSON(LS_ROUND_SKIP_EMAILS, []);
        ...
        const text = [
            ...
            `SI full hits: ${full}`,
            `No Proceed skipped emails: ${skipEmails.length}`,
            `No Proceed records: ${noProceed}`,
    ...
    function log(msg) {
        ...
        localStorage.setItem(LS_LOG, JSON.stringify(arr.slice(0, 260)));
    }
    ...
    function startNewRound() {
        ...
        localStorage.setItem(LS_ROUND_SKIP_EMAILS, JSON.stringify([]));
        localStorage.setItem(LS_SI_INDEX, "0");
        localStorage.setItem(LS_GE_INDEX, "0");
        localStorage.setItem(LS_RUNNING, "1");
        localStorage.removeItem(LS_CURRENT);

        log("New round started. SI counts and round-skip emails reset.");
        updateStatus();
        dispatchNext();
    }
    ...
    function shouldUseGE(ge) {
        ...
        if (skipEmails.includes(emailKey)) return false;

        if (shouldSkipStatus(ge.status)) {
            addRoundSkipEmail(ge.email, "Skipped by CSV Status");
            return false;
        }
        ...
        if (unlock && Date.now() < unlock.getTime()) {
            addRoundSkipEmail(ge.email, "Locked by Invited Date");
            return false;
        }
    ...
    function addRoundSkipEmail(email, reason = "No Proceed") {
        const key = String(email || "").toLowerCase();
        if (!key) return;

        const arr = getJSON(LS_ROUND_SKIP_EMAILS, []);

        if (!arr.includes(key)) {
            arr.push(key);
            localStorage.setItem(LS_ROUND_SKIP_EMAILS, JSON.stringify(arr));
            log(`Round skip email added: ${email}. Reason: ${reason}`);
        }
    }
    ...
    function runOnSIPage() {
        ...
        waitForElement(findEmailInput, input => {
            ...
            waitForElement(() => findButtonByText("next"), nextBtn => {
                ...
                addRoundSkipEmail(email, "FAILED_NO_NEXT");

                record(siId, email, {
                    status: "NO_PROCEED_SKIP_EMAIL",
                    eligible: false,
                    reason: "Next button not found; skip this email for this round",
            ...
            addRoundSkipEmail(email, "FAILED_NO_EMAIL_INPUT");

            record(siId, email, {
                status: "NO_PROCEED_SKIP_EMAIL",
                eligible: false,
                reason: "E-Mail input not found; skip this email for this round",
                pageUrl: location.href
            });
    ...
    function waitForProceedOrFailure(siId, email) {
        ...
        const timer = setInterval(() => {
            ...
            if (negative) {
                ...
                addRoundSkipEmail(email, negative);

                log(`No Proceed / Not eligible. Skip this email for this round: ${email}. Reason: ${negative}`);
            ...
            if (count > NO_PROCEED_TIMEOUT_TICKS) {
                ...
                addRoundSkipEmail(email, "No Proceed button appeared");

                log(`No Proceed. Skip this email for this round: ${email}`);

                record(siId, email, {
                    status: "NO_PROCEED_SKIP_EMAIL",
                    eligible: false,
                    reason: "No Proceed button appeared; skip this email for this round",
                    pageUrl: location.href
                });
    ...
    function clearData() {
        if (!confirm("Clear all local GE/SI data, results, counts, queue, skip emails, and log?")) return;
...
(function () {
    ...
    function startNewRound() {
        ...
        log("New round started. SI round counts reset.");
        updateStatus();
        dispatchNext();
    }
        ...
        const timer = setInterval(() => {
            ...
            if (count > 100) {
                ...
                record(siId, email, {
                    ...
                    eligible: false,
                    reason: "No Proceed or negative signal detected",
                    pageUrl: location.href
                });
    ...
    function clearData() {
        if (!confirm("Clear all local GE/SI data, results, counts, queue, and log?")) return;
